...
<div class="main-content admission-page">
    <div class="page-content">
        ...
        <?php elseif (isset($_GET['msg']) && $_GET['msg'] === 'approval_failed'): ?>
            <div class="alert alert-danger alert-dismissible fade show mb-4" role="alert" style="animation: slideDown 0.35s ease;">
        ...
        <div class="admissions-hero mb-5" style="animation: fadeInUp 0.6s cubic-bezier(0.4, 0, 0.2, 1);">
            ...
            <div class="admissions-kpi-grid">
                <div class="kpi-card kpi-total" style="animation: fadeInUp 0.7s cubic-bezier(0.4, 0, 0.2, 1) 0.1s both;">
                ...
                <div class="kpi-card kpi-pending" style="animation: fadeInUp 0.7s cubic-bezier(0.4, 0, 0.2, 1) 0.15s both;">
                    <div class="kpi-icon"><i class="bi bi-clock-history"></i></div>
                    <div class="kpi-content">
                        <p class="kpi-label">Cash Pending</p>
                        <h2 class="kpi-value"><?= htmlspecialchars(count(array_filter($admissions, fn($item) => $item['status'] === 'cash_request_sent'))) ?></h2>
                        <span class="kpi-trend">Awaiting payment</span>
                    </div>
                </div>

                <div class="kpi-card kpi-admitted" style="animation: fadeInUp 0.7s cubic-bezier(0.4, 0, 0.2, 1) 0.2s both;">
                    <div class="kpi-icon"><i class="bi bi-check-circle-fill"></i></div>
                    <div class="kpi-content">
                        <p class="kpi-label">Fully Admitted</p>
                        <h2 class="kpi-value"><?= htmlspecialchars(count(array_filter($admissions, fn($item) => $item['status'] === 'admitted'))) ?></h2>
        ...
        <div class="admissions-tracker-section" style="animation: fadeInUp 0.8s cubic-bezier(0.4, 0, 0.2, 1) 0.25s both;">
                        ...
                        <tbody>
                            <?php if (!empty($admissions)): ?>
                                <?php foreach ($admissions as $index => $admission): ?>
                                    <tr class="table-row-hover" style="animation: fadeInUp 0.5s cubic-bezier(0.4, 0, 0.2, 1) <?= ($index * 0.05) ?>s both;">
                                        <td>
                                            <span class="badge bg-light text-dark"><?= htmlspecialchars($admission['id']) ?></span>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <span class="payment-badge payment-<?= strtolower(str_replace(' ', '-', $admission['payment_method'] ?: 'none')) ?>">
                                                <?= htmlspecialchars(ucfirst($admission['payment_method'] ?: 'None')) ?>
                                            </span>
                                        </td>
                                        <td>
                                            <span class="status-badge status-<?= strtolower(str_replace('_', '-', $admission['status'])) ?>">
                                                <?= htmlspecialchars(str_replace('_', ' ', ucfirst($admission['status']))) ?>
                                            </span>
                                        </td>
                                        <td>
                                            <small class="text-secondary"><?= htmlspecialchars(date('d M Y', strtotime($admission['created_at']))) ?></small>
                                        </td>
                                        <td>
                                            <div class="action-buttons">
                                                ...
                                                <?php elseif ($admission['status'] === 'admitted' || $admission['status'] === 'form_completed'): ?>
                                                    <a href="/college/public/index.php?url=admin-admission-review&id=<?= urlencode($admission['id']) ?>" class="btn btn-sm btn-primary action-btn">
                                                        <i class="bi bi-file-check"></i> Review
                                                    </a>
                                                <?php else: ?>
                                                    <a href="/college/public/index.php?url=admission-payment&uuid=<?= urlencode($admission['uuid']) ?>" class="btn btn-sm btn-outline-primary action-btn">
